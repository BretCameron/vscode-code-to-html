  ...
  describe("buildHtml", () => {
    it("wraps output in a div", async () => {
      const files: FileEntry[] = [
        { absolutePath: "/foo/test.ts", content: "const x = 1;" },
      ];
    ...
    it("shows file path header for single file when showFilePath != none", async () => {
      const files: FileEntry[] = [
        { absolutePath: "/foo/test.ts", content: "x" },
      ];
    ...
    it("shows relative path with workspaceRoot", async () => {
      const files: FileEntry[] = [
        { absolutePath: "/workspace/src/test.ts", content: "x" },
      ];
      const html = await buildHtml(files, {
        ...defaultOptions,
    ...
    it("shows absolute path", async () => {
      const files: FileEntry[] = [
        { absolutePath: "/foo/bar/test.ts", content: "x" },
      ];
      const html = await buildHtml(files, {
        ...defaultOptions,
    ...
    it("adds line numbers when enabled", async () => {
      const files: FileEntry[] = [
        { absolutePath: "/foo/test.ts", content: "line1\nline2\nline3" },
      ];
      const html = await buildHtml(files, {
        ...defaultOptions,
        lineNumbers: true,
